...
<template>
  <div class="container">
    <div class="view-header">Intent Alignment View</div>
    <el-row type="flex" justify="space-between" style="padding: 10px">
      ...
      <el-col :span="4" style="display: flex; justify-content: center;">
        <div class="arrow-connector" style="
          position: relative;
          display: flex;
          align-items: center;
      ...
      <el-col :span="5" style="display: flex; justify-content: flex-end;">
        <el-card ref="modelUnderstandingCard" shadow="hover"
          style="width: 230px; padding: 0; border: none; position: relative;" body-style="padding: 0;">
          <div slot="header"
            style="background-color: #909aa3; color: white; padding: 10px; border-radius: 4px 4px 0 0; text-transform: uppercase; text-align: center;">
            Model Understanding
          </div>
          <div class="under-container"
            ...
            <el-input type="textarea" v-model="modelResponse" placeholder="Enter your response" :rows="5"
              style="width: 100%; max-width: 200px; height: 100%; max-height: 130px;"></el-input>
            <div
              style="background-color: #ffffff; color: #303133; padding: 10px; text-align: left; border-radius: 0 0 4px 4px; min-height: 40px;">
            ...
            <!-- Confirm 按钮 -->
            <el-button type="primary" style="position: absolute; top: 200px; left: 150px;" @click="handleConfirm"
      ...
      <el-col :span="4" style="position: relative;">
        <svg style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; border: none;" ref="svgContainer">
          <path v-for="(item, index) in topKSimilar" :key="'line-' + index"
            :d="`M0,${modelCardCenter} Q46,${modelCardCenter} 190,${getCardCenter(index)}`" fill="none" stroke="#088EA6"
            :stroke-width="getStrokeWidth(item.similarity)" stroke-opacity="0.7" style="cursor: pointer;"
            @click="handleLineClick(index, $event)" @mouseenter="showAddButton(index, $event)"
            @mouseleave="hideAddButton" />
        </svg>
        <!-- Unlinking 按钮 -->
        <el-button v-for="(item, index) in topKSimilar" :key="'unlink-' + index" v-show="activeLine === index"
          type="danger" size="default" style="position: absolute;" :style="getButtonPosition(index)"
          @click="handleUnlink(index)">
          Unlinking
        </el-button>
        <!-- 加号按钮 -->
        <img src="../assets/image/add.png" v-show="showAdd" class="add-button"
          style="position: absolute; width: 30px; height: 30px" :style="addButtonPosition"
          @click.stop="toggleHistoryWindow" />
        <!-- 自定义历史记录窗口 -->
        <div v-if="showHistoryWindow" class="history-window" :style="historyWindowPosition"
          ...
          <el-card shadow="hover" style="width: 300px; max-height: 400px; overflow-y: auto;">
            ...
            <el-checkbox-group v-model="selectedHistory">
              <el-checkbox v-for="(historyItem, idx) in historyList" :key="idx" :label="historyItem.query"
      ...
      <el-col :span="6" style="display: flex; justify-content: flex-start;">
        <el-card shadow="hover" style="width: 230px; padding: 0; border: none; position: relative;"
          body-style="padding: 0;">
          <div slot="header"
            style="background-color: #909aa3; color: white; padding: 10px; border-radius: 4px 4px 0 0; text-transform: uppercase; text-align: center;">
          ...
          <div style="position: relative; text-align: center;">
            ...
            <el-collapse v-model="activeNames">
              <el-collapse-item v-for="(item, index) in topKSimilar" :key="index" :name="index.toString()"
                :ref="'card-' + index" :title="truncateText(item.query, 20)" :style="{
                  width: '200px',
